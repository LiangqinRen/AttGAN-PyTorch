...
def check_attribute_conflict(att_batch, att_name, att_names):
    ...
    for att in att_batch:
        if att_name in ["Bald", "Receding_Hairline"] and att[att_id] != 0:
            ...
            att_name in ["Black_Hair", "Blond_Hair", "Brown_Hair", "Gray_Hair"]
            and att[att_id] != 0
        ):
...
if __name__ == "__main__":
    ...
    dataloader = data.DataLoader(dataset, batch_size=64, shuffle=False, drop_last=False)

    print("Attributes:")
    print(args.attrs)
    for x, y in dataloader:
        vutils.save_image(
            x, "test.png", nrow=8, normalize=True, value_range=(-1.0, 1.0)
        )
        print(y)
        break
    del x, y
